...
- Operator readiness is now a separate gate after infrastructure acceptance. Run `scripts/ms-qr1-operator-readiness.sh` for the table report or `scripts/ms-qr1-operator-readiness.sh --json` for machine-readable evidence before starting Phase 2 prompt, workflow and agent testing. The gate is broad, shallow and read-only: exact container health, capability-specific HTTP status contracts, required auth/config presence including `ODS_SESSION_SECRET` for Hermes, no-proxy/no-redirect loopback probes, QR1 Ollama bridge listener verification, Tailscale Serve mapping verification for canonical operator URLs, dependency state, and QR1 operator URLs derived from `MS_QR1_TAILSCALE_HOSTNAME`.
