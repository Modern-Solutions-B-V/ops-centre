...
- Langfuse PostgreSQL/ClickHouse ownership repair is also behind the quiescent-state ADR. The runbook stops and verifies derived writers before invoking `extensions/services/langfuse/hooks/post_install.sh`; the hook and Docker-rootless ownership repair both use the deployment-neutral `scripts/ods-verify-quiescent-data-writers.sh` verifier before any recursive ownership path can run, so host-agent/setup-hook callers cannot bypass the boundary and generic non-QR1 Langfuse setup does not depend on QR1 compose files.
