...
- Langfuse PostgreSQL/ClickHouse ownership repair is also behind the quiescent-state ADR. The runbook stops and verifies derived writers before invoking `extensions/services/langfuse/hooks/post_install.sh`, and the hook independently verifies quiescence for `data/langfuse` before any rootless ownership helper or host `chown` path can run, so host-agent/setup-hook callers cannot bypass the boundary.
