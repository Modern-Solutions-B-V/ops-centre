...
if [[ ! -x scripts/ms-qr1-compose-flags.sh || ! -x scripts/ods-verify-quiescent-data-writers.sh || ! -f scripts/build-installation-context.py ]]; then
...
qr1_quiescence_target_args() {
  local targets="${1:-}"
  if [[ -z "$targets" ]]; then
    printf -- '--target %s --target %s --target %s\n' "$N8N_DIR" "$PERSONA_DIR" "$LANGFUSE_DIR"
    return
  fi
  TARGETS_PAYLOAD="$targets" python3 - <<'PY'
import os
import shlex

for target in os.environ["TARGETS_PAYLOAD"].replace(",", "\n").splitlines():
    target = target.strip()
    if target:
...
writers_for_data_targets() {
  local output_kind="$1" action targets flags
  case "$output_kind" in
    services) action="writer-services" ;;
    containers) action="writer-containers" ;;
    *) echo "ERROR: invalid writer output kind: $output_kind" >&2; return 2 ;;
  esac
  # Only read-only/narrow verification actions may scope targets. prestart-init
  # always calls require_quiescent_data_writers with an empty target override so
  # no environment variable can weaken its fixed mutation boundary.
  targets="$(qr1_quiescence_target_args "${2:-${MS_QR1_QUIESCENCE_TARGETS:-}}")"
  flags="$(scripts/ms-qr1-compose-flags.sh)"
  # shellcheck disable=SC2086 # target args are generated by qr1_quiescence_target_args.
  ODS_QUIESCENCE_COMPOSE_FLAGS="$flags" ODS_QUIESCENCE_DATA_DIR="$DATA_DIR" scripts/ods-verify-quiescent-data-writers.sh "$action" --install-dir "$PWD" $targets
}
...
require_quiescent_data_writers() {
  ...
  require docker
  local targets flags
  targets="$(qr1_quiescence_target_args "${1:-}")"
  flags="$(scripts/ms-qr1-compose-flags.sh)"
  # shellcheck disable=SC2086 # target args are generated by qr1_quiescence_target_args.
  ODS_QUIESCENCE_COMPOSE_FLAGS="$flags" ODS_QUIESCENCE_DATA_DIR="$DATA_DIR" scripts/ods-verify-quiescent-data-writers.sh verify --install-dir "$PWD" $targets
}
